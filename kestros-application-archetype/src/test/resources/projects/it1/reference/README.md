...
This module contains the application layer for the Build Integration Test Project. It ties the interfaces from the api module and the services from the core module into a running site,
...
|-------|-------------|-------------------------|
| UI Framework | the framework's `kes:uiFrameworkCode` | `sample-component/bit-ui` and `sample-component/bit-versioned-ui` |
| Vendor Library | the library's `kes:libraryCode` | `apps/kestros/commons/components/content/text/bit-versioned-lib/versions/0.0.1` |
...
The library-scoped example is carried by the VERSIONED library (`build-integration-test-versioned-library`,
library code `bit-versioned-lib`), which is the library the versioned framework
...
site's theme to one belonging to `build-integration-test-versioned-framework`.
